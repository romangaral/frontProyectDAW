...
function AddComment({ postId, addComment, credentials }) {
    ...
    const onSubmit = async (e) => {
        ...
        setComment({
            ...
            let response = await fetch(`http://localhost:8090/comments/${postId}`, {
                method: "POST",
    ...
    if (credentials.role === "USER" || credentials.role === "ADMIN") {
        return (
            <Form id="commentForm" onSubmit={onSubmit}>
                ...
                <Button type="submit" variant="outline-success">PUBLICAR COMENTARIO</Button><br /><br />
            </Form>
        );
    } else {
        return <p>Registrate en nuestro Blog y podrás valorar y comentar nuestros Posts.</p>
    }
