...
function Roles({ credentials }) {
    ...
    useEffect(() => {
        fetch(`http://localhost:8090/roles?page=${page}`, {
            headers: {
    ...
    if(credentials.role === "ADMIN") {
        return (
            <Fragment>
                <RolesTable roles={roles} setRoles={setRoles} credentials={credentials}/>
                <Link className="addItem" to={"/administrador/roles/nuevo-rol"}><Button className="buttonAdd" variant="link" size="sm">Nuevo Rol</Button></Link>
