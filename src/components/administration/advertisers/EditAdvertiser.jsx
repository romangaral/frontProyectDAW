...
function EditAdvertiser({ match, credentials }) {
    ...
    useEffect(() => {
        const loadAdvertiser = async () => {
            let response = await fetch(`http://localhost:8090/advertisers/${match.params.id}`, {
                headers: {
    ...
    const editAdvertiser = async (value) => {
        let response = await fetch(`http://localhost:8090/advertisers/${advertiser.id}`, {
            method: "PUT",
                ...
                <div className="container-fluid">
                    <div className="row">
                        <div className="col-md-12">
                            <div className="formTitle"><h2>Modificando un Anunciante ya existente</h2></div>
                        </div>
                    </div>
                    <div className="row">
                        <div className="col-md-12">
                            {!advertiser && <p>Cargando datos del Anunciante</p>}
                            {advertiser && <AdvertisersForm value={advertiser} onSubmit={editAdvertiser} errors={errors} />}
                        </div>
                    </div>
                </div>
            </Fragment>
